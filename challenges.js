...
// ? I could not figure this one out after 2 hours of bainstorming, so I eventually looked at the answer for help...
...
function addList(...numb) {
  //* Returning 0 If The Array Is Empty
  if (numb === []) { return 0; }
  //* Making A Variable To Store The Sum Which is Initialy Equal To 0
  let sum = 0;
  //* Add All The Arguments Together & Return The Sum
  numb.forEach((index) => {
...
function computeRemainder(numb1, numb2) {
  if (numb2 === 0) return Infinity
  let division = numb1 / numb2;
  let wholeNumber = Math.floor(division)
  let test = wholeNumber * numb2
  let remainder = numb1 - test
  return remainder
}

console.log(computeRemainder(20, 0))
...
function range(numb1, numb2) {
  if (numb1 > numb2) return 'First argument must be less than second'
  let arr = [];
...
const reverseUpcaseString = function (string) {
  ...
  let totalReverse = arrReverse.join('')
  return totalReverse.toUpperCase();
}
...
// turn atring into an array
//loop throught the array and count how many times a letter has been used
...
function isPalindrome(argument) {
  ...
  let backwards = Array.from(argument).reverse().join('').toLowerCase().split(' ').join('');
  let forwards = backwards.split('').reverse().join('')
  if (backwards === forwards) { return true } else { return false }
}
